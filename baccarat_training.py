...
from baccarat_model import create_model, Baccarat2DCNN, predict_with_baccarat_adjustment
from baccarat_data_prep import prepare_training_data, load_all_outcomes_files
...
def load_processed_data(data_dir="processed_data"):
    ...
    try:
        ...
        from baccarat_data_prep import BaccaratDataset
...
def main(force_reprocess=False, epochs=100):
    ...
    if not force_reprocess and os.path.exists("processed_data"):
        print("Step 1: Checking for existing processed data...")
        data_loaders = load_processed_data("processed_data")
        
        if data_loaders is not None:
            train_loader, val_loader, test_loader, label_encoder = data_loaders
            print("📁 Using existing processed data")
        else:
            print("⚠️  Processed data found but corrupted, will reprocess...")
    ...
    if train_loader is None:
        ...
        if not all_outcomes:
            print("❌ No data found! Please ensure CSV files are in the 'output' directory.")
            return
